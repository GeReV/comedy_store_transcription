...
Get-ChildItem -Recurse $SourceDir | where {$_.extension -in ".mp4",".mkv",".avi"} |
Foreach-Object {
	...
	# The Hebrew-refined models are available from: https://huggingface.co/ivrit-ai
	$cmd = "ffmpeg -nostdin -threads 0 -i `"`$_`"` -f wav -ac 1 -acodec pcm_s16le -ar 16000 - |
		$WhisperExe -m ..\whisper.cpp\models\ivrit-ggml-large-v3-turbo.bin -vm ..\whisper.cpp\models\ggml-silero-v6.2.0.bin --vad -l he -ojf -osrt -of temp -pp -et 2.8 -mc 64 -f - 2>&1 |
